...
class loadData:
    def __init__(self,filename):    
        ...
        df['Abstract'] = df['Abstract'].str.replace('No abstract available','')
...
class NMFoperation:
    def __init__(self, df, n_components, n_top_words, tfidf_vectorizer):
        print("\nFitting the NMF model (Frobenius norm) with n_components=%d..."
          % (n_components))
        ...
        data_samples = list(dfFinal['Abstract'])
        ...
        self.HAll = H.copy() #HAll
        self.Wall = W.copy() #WAll
        self.dtopWords = topWords.copy() #dtopWords
...
class GetTopWords:
    def __init__(self, dfFinal, n_top_words, WAll, nmf, tfidf_vectorizer):        
        ...
        min_index, max_index = (
            ...
            max(dfFinal[dfFinal["Year"]==MAX_YEAR].index)
        )
        U = WAll[min_index: max_index, :]
        P = np.sum(U, axis = 0)/np.sum(U) 
        #n_top_words = 10
        dSup = top_words(nmf, tfidf_feature_names, n_top_words, P, year_)
...
class YearlyPopularity:
    def __init__(self, WAll, HAll, dfFinal):
        P = []
        min_index = 0
        for i in range(MIN_YEAR, MAX_YEAR+1):
            max_index = max(dfFinal[dfFinal["Year"]==i].index)
            W = WAll[min_index: max_index, :]
            # Popularity Ranking
            P.append(np.sum(W, axis = 0)/np.sum(W))
...
#         self.impactFactor = [1.39, 1.421, 1.423, 1.425, 1.42, 1.643, 1.787, 1.739, 1.907, 1.95, 1.894, 2.391, 2.665, 2.591, 2.633, 2.987, 2.898, 3.104, 3.349, 3.539]
        ...
        for i in range(MIN_YEAR,MAX_YEAR+1):
            ...
            avgTot.append(round(np.mean(Citations)/(MAX_YEAR+1-i),2))
            year2.append(i)
...
class LHSmega2ArgVal:
    def __init__(self,WAll):
        ...
        def argFunc(x):
            args = np.flip(np.argsort(x)[-NUM_TOPICS:])
            vals = np.flip(np.sort(x)[-NUM_TOPICS:])
        ...
        args = np.apply_along_axis(argFunc, 1, (WAll/LHSsum))
        vals = np.apply_along_axis(valFunc, 1, (WAll/LHSsum))
...
def Add_Citations_Corresponding_Author(dfPlot):   
    def citationsPerYear(df):
        x = round(df['Cited by']/(MAX_YEAR+1-df['Year']),2)
        return(x)
    
    t0 = time()
    dfPlot['Corresponding Author'] = dfPlot['Authors'].str.split(";").str[-1]
